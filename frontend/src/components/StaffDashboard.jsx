...
export default function StaffDashboard({ onLogout }) {
  ...
  const fetchData = () => {
    ...
    axios.get('http://127.0.0.1:8000/api/events/staff-pending/', {
      headers: { Authorization: `Bearer ${token}` }
    })
    ...
    <div>
      ...
      <nav className="navbar navbar-expand-lg navbar-light" style={{ background: '#B0E0E6', borderRadius: 8 }}>
        <div className="container-fluid">
          ...
          <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#staffNavbar">
            <span className="navbar-toggler-icon"></span>
          </button>
          <div className="collapse navbar-collapse" id="staffNavbar">
            <ul className="navbar-nav">
              <li className="nav-item">
                <a className={`nav-link${showForm ? ' active' : ''}`} href="#"
                  onClick={e => { e.preventDefault(); setShowForm(true); setActiveTab(''); }}>
                  Create New Event
                </a>
              </li>
              <li className="nav-item">
                <a className={`nav-link${activeTab === 'approved' && !showForm ? ' active' : ''}`} href="#"
                  onClick={e => { e.preventDefault(); setShowForm(false); setActiveTab('approved'); }}>
                  View Approved Events
                </a>
              </li>
              <li className="nav-item">
                <a className={`nav-link${activeTab === 'rejected' && !showForm ? ' active' : ''}`} href="#"
                  onClick={e => { e.preventDefault(); setShowForm(false); setActiveTab('rejected'); }}>
                  View Rejected Events
                </a>
              </li>
              <li className="nav-item">
                <a className={`nav-link${activeTab === 'pending' && !showForm ? ' active' : ''}`} href="#"
                  onClick={e => { e.preventDefault(); setShowForm(false); setActiveTab('pending'); }}>
                  View Pending Events
                </a>
              </li>
            </ul>
        ...
        {showForm ? (
          <div className="glass-card" style={{ maxWidth: 500, margin: "0 auto", padding: "2rem 1.5rem" }}>
            <Form onSubmit={handleCreateEvent} encType="multipart/form-data">
              <Form.Control size="sm" className="mb-2" placeholder="Title" value={title} onChange={e => setTitle(e.target.value)} required />
              <Form.Control as="textarea" rows={2} size="sm" className="mb-2" placeholder="Description" value={description} onChange={e => setDescription(e.target.value)} required />
              <Form.Control type="datetime-local" size="sm" className="mb-2" value={date} onChange={e => setDate(e.target.value)} required />
              <Form.Control type="number" size="sm" className="mb-2" placeholder="Registration Limit" value={registrationLimit} onChange={e => setRegistrationLimit(e.target.value)} required />
              <Form.Select size="sm" className="mb-2" value={category} onChange={e => setCategory(e.target.value)}>
                <option value="Tech">Tech</option>
                <option value="Cultural">Cultural</option>
                <option value="Sports">Sports</option>
              </Form.Select>
              <Form.Control type="file" size="sm" className="mb-2" accept="image/*" onChange={e => setImage(e.target.files[0])} />
              <Button type="submit" className="mt-2" size="sm" style={{ width: "100%" }}>Create</Button>
            </Form>
          </div>
        ) : activeTab === 'approved' ? (
          <div className="glass-card">
            ...
            <Table bordered hover responsive className="styled-table">
              <thead>
                <tr><th>Title</th><th>Category</th><th>Date</th><th>Limit</th><th>Registrations</th></tr>
              </thead>
              <tbody>
          ...
          <div className="glass-card">
            ...
            <Table bordered hover responsive className="styled-table">
              <thead>
                <tr><th>Title</th><th>Category</th><th>Date</th><th>Limit</th><th>Registrations</th><th>Reason</th></tr>
              </thead>
              <tbody>
          ...
          <div className="glass-card">
            ...
            <Table bordered hover responsive className="styled-table">
              <thead>
                <tr><th>Title</th><th>Category</th><th>Date</th><th>Limit</th></tr>
              </thead>
              <tbody>
